...
const FSDBalance = (props) => {
  ...
  const createContent = (items) => {
    ...
    return (
      <View style={{ width: RationValWidth(tablewidth), margin: `0 auto ${RationValWidth(32)} auto` }}>
        {data.map((item, parentInd) => (
          <View key={parentInd}
            style={[fsdStyle.tableRow1, parentInd === data.length - 1 && { borderBottomLeftRadius: 10, borderBottomRightRadius: 10 }]}
          >
