...
import { Text, View, Image, Svg, Path, Rect } from '@react-pdf/renderer';
...
function DisplaySteamData(props) {
  const { stream_ouput_data, stream_input_dict, unit_set_dict, serialNum } = props;
  ...
  return (
    <View>
      {/* Header of the page */}
      <View style={[Styles.mainHeader, { marginBottom: 15 }]} >
        <Text style={[Styles.mainHeaderText, { paddingRight: 5 }]} >{serialNum}. Detailed Stream Data</Text>
        <SubLine lineWidth='70%' lineHeight='24' />
      </View>
      {
        // Multiple tables accourding to steam numbers
        multiple_steams.map((values, i) => {
          return (
            <View key={i} break={i > 0} style={[Styles.table, { width: (227 + ((values.length - 3) * 52)), margin: '5 5' }]}>
                              ...
                              return (
                                <View style={Styles.tableRow} key={i}>
                                  <View style={[
                                    ...
                                    { paddingLeft: 10, alignItems: 'left', width: C2Width }
                                  ]}>
                                        ...
                                        {(() => {
                                          ...
                                          if (keys.key === 'Ion Concentration') {
                                            if (subkey.keyVal === 'TDS') {
                                              return (
                                                <Text style={Styles.tableCell}>{appendNumberText(stream_ouput_data[val].prop_dict.tds)}</Text>
                                              );
                                            }
                                            return (
                                              <Text style={Styles.tableCell}>{appendNumberText(stream_ouput_data[val].ppm_dict[subkey.keyVal])}</Text>
                                            );
                                          }
                                          if (keys.key === 'Stream Property') {
                                            if (subkey.keyVal === 'osmotic_pressure' || subkey.keyVal === 'conductivity') {
                                              return (
