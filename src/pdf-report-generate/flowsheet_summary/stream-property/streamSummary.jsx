...
const StreamSummary = (props) => {
    ...
    return (
      data.map((ele, index) => {
        ...
        return (
          <View key={index}
            style={[
              streamStyle.tableRow1, { width: '100%' },
            ]}
          >
            {
  ...
  const createHeader = (headerData) => {
    ...
    return (
      <View break>
        <View
          style={[
            streamStyle.tableRow, { width: '100%' },
  ...
  return (
    <View style={{ width: '100%', flexDirection: 'column', marginBottom: RationValWidth(32) }}>
      {/* <View style={[streamStyle.mainHeader, { width: '100%' }]} fixed>
        <Text style={[streamStyle.mainHeaderText, { paddingRight: 5, }]}>{serialNum}. Flowsheet Design Summary (Continued)
        </Text>
        <SubLine lineWidth='49%' lineHeight='24'  style={{marginLeft: 5}}/>
      </View> */}
