...
import { veoliaB600, RationValWidth, getFloatVal } from '../fonts/font';
    ...
    return Object.prototype.toString.call(ele) === '[object Array]' ? (
      ele.map((dta, ind) => (
        ...
        >
          <Text
            style={[
              membraneStyle.tableCell, membraneStyle.restChild, { fontFamily: veoliaB600 }
            ]}
          >
            {dta}
          </Text>
      ...
      <View style={{ height: `${RationValWidth(100 / count)}px`, width: '100%', justifyContent: 'center' }}>
        <Text
          style={[membraneStyle.tableCell, membraneStyle.restChild, { fontFamily: veoliaB600 }]}
        >
          {ele}
        </Text>
      </View>
          ...
          return (
            ...
            >
              {Object.prototype.toString.call(ele) === '[object Object]' ? (
                ...
                <View>
                  <Text style={[membraneStyle.tableCell, membraneStyle.restChild, { fontFamily: veoliaB600 }]}>
                    {ele}
                  </Text>
                </View>
              )}
                          ...
                          if (rodata[parentkey] && rodata[parentkey]['grid-data'] && rodata[parentkey]['grid-data'][typeindex] && rodata[parentkey]['grid-data'][typeindex].elementModel) {
                            return (
                              <Text style={membraneStyle.tableCell}>
                                {rodata[parentkey]['grid-data'][typeindex].elementModel.split("").map((text, textInd) => <Text key={textInd}>{text}</Text>)}
                              </Text>
                            );
                          }
                          return (
                            <Text style={membraneStyle.tableCell}>
                              {rodata[parentkey]['grid-data'][0].elementModel.split("").map((text, textInd) => <Text key={textInd}>{text}</Text>)}
                            </Text>
                          );
                        }
