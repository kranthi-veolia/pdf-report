            ...
            >
              <View
                ...
                {(() => {
                  ...
                  if (unit_type === 'CLRO') {
                    arrayData = ['stage', 'pressureVessels', 'elementsperVessel', 'elementModel', 'spacersperVessel', 'elementAge', 'permeatePressure', 'fluxAnnual', 'saltPassageAnnual'];
                  }
  ...
  return (
    <View id="dynamic-height" style={{ marginTop: 10 }}>
      ...
      {Object.keys(rodata.ro_dict).length > 0 && Object.keys(rodata.ro_dict).map((ele, ind) => {
        return (
          <View key={ind} style={{ width: RationValWidth(tablewidth), margin: '10 auto 0 auto' }} break={ind > 1}>
            ...
            <View style={[membraneStyle.mainTable, { marginTop: 10 }]}>
              {(() => {
                console.log(rodata.ro_dict[ele].unit_type);
                if (rodata.ro_dict[ele].unit_type === 'CLRO') {
                  headercount = clroHeaderVal.length;
                  return createHeader(clroHeaderVal);
                }
                headercount = herder_val.length;
                return createHeader(herder_val);
              })()}
